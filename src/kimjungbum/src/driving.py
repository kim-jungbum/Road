...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        srcPoint = np.array([[0, 0], [0, height], [width, 0], [width, height]], dtype=np.float32)
        dstPoint = np.array([[0 ,-800], [0, height], [width, -800], [width, height]], dtype=np.float32)

        matrix = cv2.getPerspectiveTransform(srcPoint, dstPoint)
        dst = cv2.warpPerspective(img, matrix, (width, height))
        gray = cv2.cvtColor(dst, cv2.COLOR_BGR2GRAY)
        dst2 = cv2.medianBlur(gray, 3)
        edges = cv2.Canny(dst2,100,200)

        lines = cv2.HoughLinesP(edges, 0.8, np.pi / 180, 90, minLineLength = 1, maxLineGap = 100)

        for i in lines:
            cv2.line(dst, (int(i[0][0]), int(i[0][1])), (int(i[0][2]), int(i[0][3])), (0, 0, 255), 2)

        cv2.imshow("CAM View", dst)
        cv2.waitKey(1)       
